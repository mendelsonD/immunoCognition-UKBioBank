...
df_interim <- df
save(df_interim, file = "UKBB_dfFormatted_full_05_02_2022.RData")
# load("UKBB_dfFormatted_full_05_02_2022.RData")
# df <- df_interim
```
...
looseCovars <- c("demo_sex_t0", "demo_ethnicity_t0",  "demo_age_assess0_t0", "demo_daysBtwAssess", "ses_townsend_t0", "med_SSRI_t0", "waistToHip", "diet_processedMeat_t0", "sleep_duration0_t0", "sleep_duration2_t2", "smoke_currently_t0", "exercise_IPAQActivityGroup_t0")
...
cut2(df$demo_age_assess0_t0, g = 3)
# Demographics
df_old <- df %>% filter(df$demo_age_assess0_t0 < ) # keeps only participants in the top tertile of age
a
...
write.csv(df_noDx, file = paste("UKBB_dfForAnal_excludingDx", date, ".csv", sep = ""))
rm(df_noDx)

df_onlyDx <- df %>% filter(anyDxOfInterest == 1) # remove participants without diagnoses from dataframe 
write.csv(df_onlyDx, file = paste("UKBB_dfForAnal_onlyDx", date, ".csv", sep = ""))
rm(df_onlyDx)

# Medications
df_noSSRI <- df %>% filter(med_SSRI_t0 == 0 && med_SSRI_t2 == 0) # remove participants taking meds from dataframe 
write.csv(df_noSSRI, file = paste("UKBB_dfForAnal_excludingSSRI", date, ".csv", sep = ""))
rm(df_noSSRI)

df_onlySSRI <- df %>% filter(med_SSRI_t0 == 1 || med_SSRI_t2 == 1) # remove participants taking meds from dataframe 
write.csv(df_onlySRRI, file = paste("UKBB_dfForAnal_onlySSRIUsers", date, ".csv", sep = ""))
rm(df_onlySRRI)

df_noMed <- df %>% filter(med_AnyOfInterest0 == 0 && med_AnyOfInterest2 == 0) # remove participants taking meds from dataframe 
write.csv(df_noMed, file = paste("UKBB_dfForAnal_excludingMed", date, ".csv", sep = ""))
rm(df_noMed)

df_onlyMedUsers <- df %>% filter(med_AnyOfInterest0 == 1 || med_AnyOfInterest2 == 1) # remove participants not taking meds from dataframe 
write.csv(df_onlyMedUsers, file = paste("UKBB_dfForAnal_onlyMed", date, ".csv", sep = ""))
rm(df_onlyMedUsers)
