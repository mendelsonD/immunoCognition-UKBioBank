...
df <- readr::type_convert(df) # automatically detect
df_interim <- df
save(df_interim, file = "UKBB_dfFormatted_full.RData")
```
...
brainVars_specific <- which(colnames(df) %in% c("brain_vol_brainSegNoVent_t2", "brain_vol_hippocamp_L_t2", "brain_vol_hippocamp_R_t2"))
...
finalCovars <- which(colnames(df) %in% c("demo_sex_t0", "demo_ethnicity_t0", "demo_age_assess0_t0_z", "demo_daysBtwAssess_z", "ses_townsend_t0_z", "weight_waistToHip_z", "sleep_duration0_t0_z", "exercise_IPAQActivityGroup_t0"))
...
essentialVars <- c(which(colnames(df) %in% c("demo_sex_t0", "demo_ethnicity_t0", "demo_age_assess0_t0_z", "demo_daysBtwAssess_z", "ses_townsend_t0_z", "weight_waistToHip_z", "sleep_duration0_t0_z", "exercise_IPAQActivityGroup_t0")), completionTimeVars, crpAliqVars, cogPCAVars, brainVars_specific) # list of variable names that must be complete in order for case to be retained
...
which(colnames(df) == "diet_alcohol_yesterdayIntake_t0")
normalityPlots <- mapply(normalCheck, df[c(numVarColsToCompare, 74)], colName = colnames(df[c(numVarColsToCompare, 74)]), colNum = c(numVarColsToCompare, 74))
...
nonParametricVars <- c(3,8,9,12,38,40,41,69,74,105)
```
...
summaryMissing <- list()
nonPara <- c()
nonParaToCompare <- c("demo_age_assess0_t0", "demo_age_assess2_t2", "demo_daysBtwAssess", "ses_townsend_t0", "crp_timeFasting_t0", "crp_timeCollected_t0", "crp_aliquotdelay", "crp_aliquot_t0", "weight_BMI0_t0", "weight_BMI2_t2", "diet_alcohol_yesterdayIntake_t0", "sleep_duration0_t0")
...
varsOfInterest <- c(crp_fctrCol, brainVarsOfInt, cogPCOutputs, covars) # list of variables used in correlation/mediation analyses
...
nonNormalVarsOfInt <- c(9,12,13,40,126,127,128)
...
revisedvarsOfInterest <- c(varsOfInterest,normCorrected)
...
df_z <- df %>% mutate("{colnames(df[revisedVarsColnums])}_z" = across(.cols = all_of(revisedVarsColnums), .fns = scale))
