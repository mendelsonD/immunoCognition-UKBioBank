...
df <- df %>%
  ...
  mutate(demo_ethnicity_t0 = case_when( # field 4282 ('cog_numMem_maxDigitRemem_t2'), -1 = abandoned
    ...
    demo_ethnicity_t0 == "Bangladeshi" ~ "South Asian",
    demo_ethnicity_t0 == "Asian or Asian British" ~ "Any other Asian background",
...
nonNormalVarsOfInt <- c(9,12,13,40, 106, 107, 108)
...
for(i in numRevisedVarsOfInt){
  if(is.null(i) == FALSE){
    print(colnames(numRevisedVarsOfInt[i]))
  }
}
df_z <- df %>% mutate("{colnames(df[revisedvarsOfInterest])}_z" = across(.cols = colnames(df[revisedvarsOfInterest]), .fns = scale))
