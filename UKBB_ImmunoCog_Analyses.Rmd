...
runRMediation <- function(df, X, Y, M, covars, contrasts){
  require(RMediation)
...
varsOfInterest <- c(crp, crp_fctrCol, crp_log, crp_log_z, brainVars_specific_z, cogPCOutputs, numMemCol, covars, covars_cog) # list of variables used in correlation/mediation analyses
...
save(df_complete, file = "UKBB_data_prelimAnalyses_03_23_2022")
nrow(df_complete)
nrow(df)

````
## correlational analyses
x = CRP; Y1 = cog_1_z; Y2 = cog_2_z; Y3 = cog_3_z;
### No covariates
!!! In outputs, specify what subset of participants it is for !!!
```{r corAnalyses_noC, echo=FALSE}
# Run models ---------------
mainCor_noC_log_cog1 <- lm(cog_PC_1_z ~ crp_log_z, data = df_complete)
mainCor_noC_fctr_cog1 <- lm(cog_PC_1_z ~ crp_aliquot_fctr, data = df_complete, 
                            contrasts=list(crp_aliquot_fctr = "contr.treatment"))

mainCor_noC_log_cog2 <- lm(cog_PC_2_z ~ crp_log_z, data = df_complete)
mainCor_noC_fctr_cog2 <- lm(cog_PC_2_z ~ crp_aliquot_fctr, data = df_complete, 
                            contrasts=list(crp_aliquot_fctr = "contr.treatment"))

mainCor_noC_log_cog3 <- lm(cog_PC_3_z ~ crp_log_z, data = df_complete)
mainCor_noC_fctr_cog3 <- lm(cog_PC_3_z ~ crp_aliquot_fctr, data = df_complete, 
                            contrasts=list(crp_aliquot_fctr = "contr.treatment"))

mainCor_noC_log_numMem <- lm(cog_numMem_maxDigitRemem_t2 ~ crp_log_z, data = df_complete)
mainCor_noC_fctr_numMem <- lm(cog_numMem_maxDigitRemem_t2 ~ crp_aliquot_fctr, data = df_complete, 
                              contrasts=list(crp_aliquot_fctr = "contr.treatment"))
...
### With covariates
x = CRP; Y1 = cog_1_z; Y2 = cog_2_z; Y3 = cog_3_z; C = sex, current smoking, processedMeat, ethnicity, exercise, SSRI, waistToHip, daysBTWAssess, age_assess, sleep_duration
```{r corAnalyses_C, echo=FALSE}
# Create LM expressions --------
withC_log_cog1 <- createLMExpression(varNames = colnames(df_complete), X = "crp_log_z", C = colnames(df[covars]), M = "", Y = "cog_PC_1_z")
withC_fctr_cog1 <- createLMExpression(varNames = colnames(df_complete), X = "crp_aliquot_fctr", C = colnames(df[covars]), M = "", Y = "cog_PC_1_z")

withC_log_cog2 <- createLMExpression(varNames = colnames(df_complete), X = "crp_log_z", C = colnames(df[covars]), M = "", Y = "cog_PC_2_z")
withC_fctr_cog2 <- createLMExpression(varNames = colnames(df_complete), X = "crp_aliquot_fctr", C = colnames(df[covars]), M = "", Y = "cog_PC_2_z")

withC_log_cog3 <- createLMExpression(varNames = colnames(df_complete), X = "crp_log_z", C = colnames(df[covars]), M = "", Y = "cog_PC_3_z")
withC_fctr_cog3 <- createLMExpression(varNames = colnames(df_complete), X = "crp_aliquot_fctr", C = colnames(df[covars]), M = "", Y = "cog_PC_3_z")

withC_log_numMem <- createLMExpression(varNames = colnames(df_complete), X = "crp_log_z", C = colnames(df[covars]), M = "", Y = "cog_numMem_maxDigitRemem_t2")
...
mainCor_C_log_cog1 <- lm(eval(parse(text = withC_log_cog1)), data = df_complete, contrasts=ordinalFactorContrasts)
...
mainCor_C_log_cog2 <- lm(eval(parse(text = withC_log_cog2)), data = df_complete, contrasts=ordinalFactorContrasts)
...
mainCor_C_log_cog3 <- lm(eval(parse(text = withC_log_cog3)), data = df_complete, contrasts=ordinalFactorContrasts)
...
mainCor_C_log_numMem <- lm(eval(parse(text = withC_log_numMem)), data = df_complete, contrasts=ordinalFactorContrasts)
...
# Summarise --------
list_models_C_log <- c(withC_log_cog1, summary(mainCor_C_log_cog1), withC_log_cog2, summary(mainCor_C_log_cog2), withC_log_cog3, summary(mainCor_C_log_cog3), withC_log_numMem, summary(mainCor_C_log_numMem)) # list of c(ovariate crp_log models. Name of the LM is included before the summary as it is passed in as a variable during the LM execution.
...
# Create LM expressions --------
withC_log_cogTest <- createLMExpression(varNames = colnames(df_complete), X = "", C = colnames(df[covarsRevised1]), M = "", Y = "cog_PC_1_z")

withC_log_cog1 <- createLMExpression(varNames = colnames(df_complete), X = "crp_log_z", C = colnames(df[covarsRevised1]), M = "", Y = "cog_PC_1_z")

withC_log_cog2 <- createLMExpression(varNames = colnames(df_complete), X = "crp_log_z", C = colnames(df[covarsRevised1]), M = "", Y = "cog_PC_2_z")

withC_log_cog3 <- createLMExpression(varNames = colnames(df_complete), X = "crp_log_z", C = colnames(df[covarsRevised1]), M = "", Y = "cog_PC_3_z")

withC_log_numMem <- createLMExpression(varNames = colnames(df_complete), X = "crp_log_z", C = colnames(df[covarsRevised1]), M = "", Y = "cog_numMem_maxDigitRemem_t2")
...
mainCor_C_log_cogTest <- lm(eval(parse(text = withC_log_cogTest)), data = df_complete, contrasts=ordinalFactorContrasts)
summary(mainCor_C_log_cogTest)
mainCor_C_log_cog1 <- lm(eval(parse(text = withC_log_cog1)), data = df_complete, contrasts=ordinalFactorContrasts)
mainCor_C_fctr_cog1 <- lm(eval(parse(text = withC_fctr_cog1)), data = df_complete, contrasts=c(crp_aliquot_fctr = "contr.treatment",ordinalFactorContrasts))
...
avPlots(mainCor_C_log_cogTest, pt.wts = T)
avPlots(mainCor_C_log_cog1, pt.wts = T) # if slope is 0 then the predictor is not predicting unique variance in the model. Slope of lines should be equivalent to the regression coefs in the model.
avPlots(mainCor_C_log_cog2, pt.wts = T)
avPlots(mainCor_C_log_cog3, pt.wts = T)
...
## Mediation analyses
### No covariates
x = CRP; Y0i = cog_1_z; Y0ii = cog_2_z; Y0iii = cog_3_z; M0: brain_vol_brainSegNoVent_t2_z; M1a: brain_vol_hippocamp_L_t2_z; M2a: brain_vol_hippocamp_R_t2_z;
```{r medAnalyses, echo=FALSE}
# Run mediation models
medModel0_noC <- runRMediation(df = df_complete, X = "crp_log_z", Y = "cog_PC_1_z", M = "brain_vol_brainSegNoVent_t2_z", covars = "", contrasts = c(""))

medModel0_C <- runRMediation(df = df_complete, X = "crp_log_z", Y = "cog_PC_1_z", M = "brain_vol_brainSegNoVent_t2_z", covars = colnames(df[covars]), contrasts = ordinalFactorContrasts)
paste("indirect effect = ", round(medModel0_C$Estimate, 3), " 95%CI[", round(medModel0_C$`97.5% CI`[1], 3), ", ", round(medModel0_C$`97.5% CI`[2], 3), "]", sep = "")

medModel1a_noC <- runRMediation(df = df_complete, X = "crp_log_z", Y = "cog_numMem_maxDigitRemem_t2", M = "brain_vol_hippocamp_L_t2_z", covars = "", contrasts = c(""))
medModel1a_C <- runRMediation(df = df_complete, X = "crp_log_z", Y = "cog_numMem_maxDigitRemem_t2", M = "brain_vol_hippocamp_L_t2_z", covars = colnames(df[covars]), contrasts = ordinalFactorContrasts)

medModel1b_noC <- runRMediation(df = df_complete, X = "crp_log_z", Y = "cog_numMem_maxDigitRemem_t2", M = "brain_vol_hippocamp_R_t2_z", covars = "", contrasts = c(""))

medModel1b_C <- runRMediation(df = df_complete, X = "crp_log_z", Y = "cog_numMem_maxDigitRemem_t2", M = "brain_vol_hippocamp_R_t2_z", covars = colnames(df[covars]), contrasts = ordinalFactorContrasts)
...
### With covariates
x = CRP; Y0i = cog_1_z; Y0ii = cog_2_z; Y0iii = cog_3_z; M0: brain_vol_brainSegNoVent_t2_z; M1a: brain_vol_hippocamp_L_t2_z; M2a: brain_vol_hippocamp_R_t2_z; covars = demo_sex_t0, demo_ethnicity_t0, demo_age_assess0_t0, demo_daysBtwAssess_z, waistToHip_z
```{r medAnalyses, echo=FALSE}
model0 <- runRMediation(df = df_complete, X = "crp_log_z", Y = "cog_PC_1_z", M = "brain_vol_brainSegNoVent_t2_z", covars = colnames(df[covarsRevised1]), contrasts = c("")) # model between CRP, whole brain volume, general cog
model1a <- runRMediation(df = df, X = X, Y = Y, M = "brain_vol-hippocamp-L", covars = covarsRevised1, contrasts = c("")) # model between CRP, L hippocamp, memory
model1b <- runRMediation(df = df, X = X, Y = Y, M = "brain_vol-hippocamp-R", covars = covarsRevised1, contrasts = c("")) # model between CRP, L hippocamp, memory
