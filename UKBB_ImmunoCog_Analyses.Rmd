...
load("UKBB_dfForAnal_03_31_2022.Rdata")
...
covars <-  which(colnames(df) %in% c("demo_sex_t0", "demo_ethnicity_t0", "demo_age_assess0_t0", "demo_daysBtwAssess_z", "ses_townsend_t0_z", "med_SSRI0", "weight_waistToHip_z", "diet_processedMeat_t0", "sleep_duration0_t0_z", "smoke_currently_t0", "exercise_IPAQActivityGroup_t0"))
revisedCovars <-  which(colnames(df) %in% c("demo_sex_t0", "demo_ethnicity_t0", "demo_age_assess0_t0", "demo_daysBtwAssess_z"))
...
df_small_allC <- df[c(1,varsOfInterest_allC, which(colnames(df) == "missingEssentialVar"))]
df_small <- df[c(1,varsOfInterest_revC, which(colnames(df) == "missingEssentialVar"))]


essentialVarsAnalysis <- c(cogOutcomes, cogOutcomes_fctr,  colnames(df[brainVars_specific_z]), colnames(df[revisedCovars]), colnames(df[covars_brain]))
...
avPlots(mainCor_C_cog1, pt.wts = T) # if slope is 0 then the predictor is not predicting unique variance in the model. Slope of lines should be equivalent to the regression coefs in the model.
avPlots(mainCor_C_cog2, pt.wts = T)
avPlots(mainCor_C_cog3, pt.wts = T)
...
### Scatterplots ---------
length(df$crp_log)
df$cog_PC_1_z <- scale(df$cog_PC_1)
plot(df$crp_log, df$cog_PC_1_z)
df_complete$demo_age_assess0_t0_z <- scale(df_complete$demo_age_assess0_t0)
df$demo_daysBtwAssess_z <- df$demo_demo_daysBtwAssess
runCor(df = df, X = "crp_log_z", Y ="cog_PC_1_z", covars = c("demo_sex_t0", "demo_ethnicity_t0", "demo_age_assess0_t0_z", "demo_daysBtwAssess_z"), M = "", YisFactor = F, contrasts = c(), name = "cor_revC")
df$crp_log
ggplot(data = df, aes(x=crp_log, y=cog_PC_1_z)) + # ideally: plot PC1 vs CRP, with 2 x-axes: one for the log value, the other for the raw CRP values; and 2 ablines: one for the model with no covars, the second with the revised covariates
                  geom_point(alpha = .5, size = 1, color = "orange") +
                  labs(title = "Cognitive component 1 vs log(CRP) when controling for covariates", 
                      x = "log of crp aliquot", 
                      y = "Cognition component 1 (z score)") + 
                  geom_abline(aes(intercept = 7.944e-18, slope = -7.499e-02, color="no covariates")) +
                  geom_abline(aes(intercept = -0.71221, slope = -0.08176, color="revised covariates")) +
                  labs(color=NULL)
                  annotate(geom = "text", x = perc, y = -0.1*yrng, label = labs, size=4) + # should add second axis. SEe https://stackoverflow.com/questions/22285360/add-second-x-axis-in-ggplot2
                  annotate(geom = "text", x=0, y=-0.16*yrng, label="CRP value", size=4.5)
