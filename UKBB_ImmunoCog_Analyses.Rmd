...
# df <- read.csv("./UKBB_dfForAnal_03_21_2022.csv", ) # import df
...
varsOfInterest <- c(crp, crp_fctrCol, crp_log, crp_log_z, brainVars_specific_z, cogPCOutputs, cogVars_t2_z, cogVars_t2_fctr, revisedCovars, covars_cog) # list of variables used in correlation/mediation analyses
...
listAnalyses_cor_noC <- list()
mainCor_noC <- list()
for(i in cogOutcomes){
  expression <- createLMExpression(varNames = colnames(df), X = "crp_log_z", Y = i, C = "", M = "")
  model <- lm(eval(parse(text = expression)), data = df_complete)
  listAnalyses_cor_noC <- c(listAnalyses, expression)
...
for(i in cogOutcomes_fctr){
  ...
  listAnalyses_cor_noC <- c(listAnalyses_cor_noC, expression)
  individualAnalysis <- c(expression, model)
  mainCor_noC <- c(mainCor_noC, individualAnalysis)
  print(paste("---- ", expression, " ----", sep = ""))
  print(summary(model))
...
### With covariates
x = CRP; Y1 = cog_1_z; Y2 = cog_2_z; Y3 = cog_3_z; Covariates = `r colnames(df[revisedCovars])`
...
mainCor_C <- list()
for(i in cogOutcomes){
  expression <- createLMExpression(varNames = colnames(df), X = "crp_log_z", Y = i, C = colnames(df[revisedCovars]), M = "")
  model <- lm(eval(parse(text = expression)), data = df_complete)
...
for(med in colnames(df[mediators])){
  for(i in cogOutcomes){
    model <- runRMediation(df = df_complete, X = "crp_log_z", Y = i, M = med, covars = "", contrasts = c(""))
  ...
  # for(i in cogOutcomes_fctr){
  #   expression <- createLMExpression(varNames = colnames(df), X = "crp_log_z", Y = i, M = med, C = "")
  #   model <- glm(formula = eval(parse(text = expression)), data = df_complete, family = "binomial")
  #   listAnalyses_med_noC <- c(listAnalyses_med_noC, expression)
  #   individualAnalysis <- c(expression, model)
  #   mainMed_noC <- c(mainMed_noC, individualAnalysis)
  #   print(paste("---- ", expression, " ----", sep = ""))
  #   print(summary(model))
  # }
}
...
```{r medAnalyses, echo=FALSE}
covarsMed <- c(covarsRevised1, covars_cog )
# Add  cognition only covars
...
for(med in colnames(df[mediators])){
  for(i in cogOutcomes){
    model <- runRMediation(df = df_complete, X = "crp_log_z", Y = i, M = med, covars = colnames(df[covarsMed]), contrasts = c(""))
  ...
  #   expression <- createLMExpression(varNames = colnames(df), X = "crp_log_z", Y = i, M = med, C = "")
  #   model <- glm(formula = eval(parse(text = expression)), data = df_complete, family = "binomial")
  #   listAnalyses_med_C <- c(listAnalyses_med_C, expression)
  #   individualAnalysis <- c(expression, model)
  #   mainMed_C <- c(mainMed_C, individualAnalysis)
  #   print(paste("---- ", expression, " ----", sep = ""))
  #   print(summary(model))
  # }
}
