...
runRMediation <- function(df, X, Y, M, covars, contrasts, YisFactor, name, alpha){
  ...
  med <- medci(mu.x = a, mu.y = b, se.x = a.se, se.y = b.se, rho = 0, alpha = alpha, plot = T, plotCI = T, type = "asymp")
  capture.output(
    cat(" -- Linear Model Analysis Output -- "),
...
load("UKBB_dfForAnal_04_01_2022.Rdata")
...
covars <-  which(colnames(df) %in% c("demo_sex_t0", "demo_ethnicity_t0", "demo_age_assess0_t0_z", "demo_daysBtwAssess_z", "ses_townsend_t0_z", "med_SSRI0", "weight_waistToHip_z", "diet_processedMeat_t0", "sleep_duration0_t0_z", "smoke_currently_t0", "exercise_IPAQActivityGroup_t0", "cog_hourCompleted_z"))
duplicateVarNames <- ends_with(".1", vars = colnames(df[covars]))
covars <- covars[-duplicateVarNames]

revisedCovars <-  which(colnames(df) %in% c("demo_sex_t0", "demo_ethnicity_t0", "demo_age_assess0_t0_z", "demo_daysBtwAssess_z", "ses_townsend_t0_z", "weight_waistToHip_z", "sleep_duration0_t0_z", "exercise_IPAQActivityGroup_t0"))
duplicateVarNames <- ends_with(".1", vars = colnames(df[revisedCovars]))
revisedCovars <- revisedCovars[-duplicateVarNames]

revisedCovars1 <-  which(colnames(df) %in% c("demo_sex_t0", "demo_ethnicity_t0", "demo_age_assess0_t0_z", "demo_daysBtwAssess_z", "ses_townsend_t0_z", "weight_waistToHip_z", "sleep_duration0_t0_z"))
duplicateVarNames <- ends_with(".1", vars = colnames(df[revisedCovars1]))
revisedCovars1 <- revisedCovars1[-duplicateVarNames]

covars_brain <- which(colnames(df) %in% c("hand_t0", "brain_headScale_t2"))

varsOfInterest_allC <- c(crp, crp_fctrCol, crp_log, crp_log_z, brainVars_specific_z, cogPCOutputs, cogVars_t2_z, cogVars_t2_fctr, covars) # list of variables used in correlation/mediation analyses
varsOfInterest_revC <- c(crp, crp_fctrCol, crp_log, crp_log_z, brainVars_specific_z, cogPCOutputs, cogVars_t2_z, cogVars_t2_fctr, revisedCovars, covars_brain) # list of variables used in correlation/mediation analyses
colnames(df_small)


essentialVarsAnalysis <- c(colnames(df[cogPCOutputs_z]), colnames(df[crp]), colnames(df[brainVars_specific_z]), colnames(df[revisedCovars]), colnames(df[covars_brain]))
essentialVarsAnalysisNum <- c(crp, crp_log_z, brainVars_specific_z, cogPCOutputs_z, revisedCovars, covars_brain)
save(essentialVarsAnalysis, file = "revisedEssentialVarsAnalysis.RData")

df_small_allC <- df[c(1,essentialVarsAnalysisNum, which(colnames(df) == "missingEssentialVar"))]
df_small <- df[c(1,varsOfInterest_revC, which(colnames(df) == "missingEssentialVar"))]

```

````{r missingVars, echo=FALSE}
df_complete <- df_small_allC[complete.cases(df_small_allC[c(essentialVarsAnalysis, crp_log)]),]
...
df_complete <- df_small[complete.cases(df_small[c("ses_townsend_t0_z", "sleep_duration0_t0_z", "exercise_IPAQActivityGroup_t0")]),]
df_complete <- df_small[complete.cases(df_small[essentialVarsAnalysis]),]
save(df_complete, file = paste("UKBB_data_corAnalyses_allC", date, ".csv", sep = ""))
...
individualAnalysis <- runCor(df = df_complete, X = "crp_log_z", Y = "cog_PC_1_z", covars = "", M = "", YisFactor = F, name = analysisName, contrasts = c())
...
Indep var: 'crp_log_z'; 
Outcomes: `r colnames(df[c(cogOutcomes, cogOutcomes_fctr)])`
Covariates = `r colnames(df[covars])`
...
individualAnalysis <- list()
mainCor_C <- list()
analysisName <- "UKBB_Anal_cor_allC"

PC1_allC <- runCor(df = df_small_allC, X = "crp_log_z", Y = "cog_PC_1_z", covars = colnames(df[covars]), M = "", YisFactor = F, name = analysisName, contrasts = ordinalFactorContrasts)
capture.output(summary(PC1_allC), file = glue("UKBB_cor_allC_PC1_z_{date}.txt"))
summary(PC1_allC)

# for numeric variables
for(i in cogOutcomes){
  analysisName <- glue("{analysisName}_{i}")
  individualAnalysis <- runCor(df = df_small_allC, X = "crp_log_z", Y = i, covars = colnames(df[covars]), M = "", YisFactor = F, name = analysisName, contrasts = ordinalFactorContrasts)
...
for(i in cogOutcomes_fctr){
  analysisName <- glue("{analysisName}_{i}")
  individualAnalysis <- runCor(df = df_small_allC, X = "crp_log_z", Y = i, covars = colnames(df[covars]), M = "", YisFactor = T, name = analysisName, contrasts = ordinalFactorContrasts)
  mainCor_allC <- c(mainCor_C, individualAnalysis)
  capture.output(summary(individualAnalysis), file = glue("{analysisName}_{date}.txt"))
  # print(summary(listAnalyses_cor_noC))
  analysisName <- "UKBB_Anal_cor_allC"
...
Covars: `r colnames(df[revisedCovars])`
``` {r saveCorAnalyses, echo=FALSE}
# ordinalFactorContrasts <- list(diet_processedMeat_t0 = "contr.treatment",
#                                exercise_IPAQActivityGroup_t0 = "contr.treatment",
#                                sleep_duration0_t0_tert = "contr.treatment",
...
individualAnalysis <- list()
mainCor_C <- list()
analysisName <- "UKBB_Anal_cor_rev1C"

PC1_revC <- runCor(df = df_small_allC, X = "crp_log_z", Y = "cog_PC_1_z", covars = colnames(df[revisedCovars]), M = "", YisFactor = F, name = analysisName, contrasts = ordinalFactorContrasts)
PC1_rev1C <- runCor(df = df_small_allC, X = "crp_log_z", Y = "cog_PC_1_z", covars = colnames(df[revisedCovars1]), M = "", YisFactor = F, name = analysisName, contrasts = ordinalFactorContrasts)

for(i in cogOutcomes){
  analysisName <- glue("{analysisName}_{i}")
  individualAnalysis <- runCor(df = df_complete, X = "crp_log_z", Y = i, covars = colnames(df[revisedCovars1]), M = "", YisFactor = F, name = analysisName, contrasts = ordinalFactorContrasts)
  mainCor_revC <- c(mainCor_C, individualAnalysis)
  # print(summary(listAnalyses_cor_noC))
  analysisName <- "UKBB_Anal_cor_revC"
}

for(i in cogOutcomes_fctr){
  analysisName <- glue("{analysisName}_{i}")
  individualAnalysis <- runCor(df = df_complete, X = "crp_log_z", Y = i, covars = colnames(df[revisedCovars1]), M = "", YisFactor = T, name = analysisName, contrasts = ordinalFactorContrasts)
  mainCor_revC <- c(mainCor_C, individualAnalysis)
  # print(summary(listAnalyses_cor_noC))
  analysisName <- "UKBB_Anal_cor_revC"
}
...
avPlots(PC1_revC, pt.wts = T) # if slope is 0 then the predictor is not predicting unique variance in the model. Slope of lines should be equivalent to the regression coefs in the model.
avPlots(mainCor_C_cog2, pt.wts = T)
avPlots(mainCor_C_cog3, pt.wts = T)
...
for(med in colnames(df[mediators])){
  for(i in cogOutcomes){
    ...
    model <- runRMediation(df = df_complete, X = "crp_log_z", Y = i, M = med, covars = "", contrasts = c(""), YisFactor = F, name = analysisName, alpha = .001)
  ...
  for(i in cogOutcomes_fctr){
     analysisName <- glue("{analysisName}_{med}_{i}")
    model <- runRMediation(df = df_complete, X = "crp_log_z", Y = i, M = med, covars = "", contrasts = c(""), YisFactor = T, name = analysisName, alpha = .001)
    # mainMed_noC <- c(mainMed_noC, model)
    # print(paste("indirect effect = ", round(model$Estimate, 3), " 95%CI[", round(model$`97.5% CI`[1], 3), ", ", round(model$`97.5% CI`[2], 3), "]", sep = ""))
    analysisName <- glue("UKBB_Anal_med_noC")
  }
...
```{r medAnalyses, echo=FALSE}
covarsMed <- c(revisedCovars, covars_brain) # Add  cognition only covars
# colnames(df[covarsMed ])

individualAnalysis <- list()
listAnalyses_med_C <- list()
mainMed_C <- list()
analysisName <- "UKBB_Anal_med_revC"

for(med in colnames(df[mediators])){
  for(i in cogOutcomes){
    analysisName <- glue("{analysisName}_{med}_{i}")
    model <- runRMediation(df = df_complete, X = "crp_log_z", Y = i, M = med, covars = colnames(df[covarsMed]), contrasts = ordinalFactorContrasts, YisFactor = F, name = analysisName, alpha = .05)
  ...
  for(i in cogOutcomes_fctr){
    analysisName <- glue("{analysisName}_{med}_{i}")
    model <- runRMediation(df = df_complete, X = "crp_log_z", Y = i, M = med, covars = colnames(df[covarsMed]), contrasts = ordinalFactorContrasts, YisFactor = T, name = analysisName, alpha = .05)
    mainMed_C <- c(mainMed_C, model)
    # print(paste("indirect effect = ", round(model$Estimate, 3), " 95%CI[", round(model$`97.5% CI`[1], 3), ", ", round(model$`97.5% CI`[2], 3), "]", sep = ""))
    analysisName <- glue("UKBB_Anal_med_revC")
  }
...
ggplot(data = df_complete, aes(x=crp_log_z, y=cog_PC_1_z)) + # ideally: plot PC1 vs CRP, with 2 x-axes: one for the log value, the other for the raw CRP values; and 2 ablines: one for the model with no covars, the second with the revised covariates
                  ...
                  geom_abline(aes(intercept = 0, slope = -7.499e-02, colour="no covariates"), size = 1, alpha = .7) +                
                  geom_abline(aes(intercept = 0, slope = -0.020185, colour="with covariates"), size = 1, alpha = .7) +                
                  labs(color=NULL) + 
                  scale_x_continuous(
...
# mediation diagram
medDigram_wbv <- med_diagram(mediation_data =  data.frame(
    main = "CRP on general cognition via Whole brain volume",  
    lab_x   = "C-Reactive Protein",
    lab_m   = "Total brain volume",
    lab_y   = "General cognitive performance",
    coef_xm = "-0.01**, p = .003",
    coef_my = "0.17***, p < 2e-16",
    coef_xy = "direct effect: -.01*, p = .05",
    coef_indEffect = paste("indirect effect = -0.002*, 95%CI[-0.003, -0.001]")))
# * sig at .05, ** at .01, *** at .001

medDiagram_HippoL <- med_diagram(mediation_data =  data.frame(
    main = "CRP on general cognition via left hippocampal volume",  
    lab_x   = "C-Reactive Protein",
    lab_m   = "Left hippocampal volume",
    lab_y   = "General cognitive performance",
    coef_xm = "0.01, p = .34",
    coef_my = "0.05***, p = 4e-9",
    coef_xy = "direct effect: -0.02*, p = .02",
    coef_indEffect = paste("indirect effect < 0.001, 95%CI[-0.0003, 0.001]")))

medDiagram_HippoR <- med_diagram(mediation_data =  data.frame(
    main = "CRP on general cognition via right hippocampal volume",  
    lab_x   = "C-Reactive Protein",
    lab_m   = "Right hippocampal volume",
    lab_y   = "General cognitive performance",
    coef_xm = "0.01, p = .39",
    coef_my = "0.04***, p = 3e-7",
    coef_xy = "direct effect: -0.02*, p = .03",
    coef_indEffect = paste("indirect effect = 0.0002, 95%CI[-0.0003, 0.001]")))
