...
for(i in vars){
  if(is.numeric(as.matrix(df[[i]])) == FALSE){ # checks that variables are numeric
    ...
    } else if(round(mean(as.matrix(df[i]), na.rm = T), 4) != 0 | round(sd(as.matrix(df[i]), na.rm = T),4) != 1){
      df[i] <- scale(df[i])
...
prcomp(as.matrix(df[,vars]), center = TRUE, scale = TRUE)
typeof(as.matrix(df[vars]))
df <- df %>% 
  tibble()
cor(tibble(df[,vars]))
typeof(df[,vars])
eigen(cor(as.matrix(df[vars])))$values
View(df[vars])

```
see https://www.datacamp.com/community/tutorials/pca-analysis-r
```{r PCAFx, echo=FALSE}
PCA <- function(df, vars){
  # PCA (see 536 lecture notes week 10) ---------
  ## assumption checks ---------
  rejectVars <- c()
  for(i in vars){
    if(is.numeric(df[[i]]) == FALSE){ # checks that variables are numeric
      rejectVars <- append(rejectVars, i)
    } else {
      if(round(mean(df[i]), 4) != 0 | round(sd(df[i] != 1),4) != 1){
        df[i] <- scale(df[i])
      }
    }
  }
  ...
  z_eigenValues <- eigen(cor(df[vars]))$values
  for(i in z_eigenValues){
    if(i > 1){
  ...
  z_PCAvarExplained <- z_eigenValues/length(df[vars])
...
# df_all <- read_csv("/home/doodlefish/Documents/Research/LepageLab/immunologyAndSz/Data/CCData/Daniel_2022-03-15.csv") # import df
...
# write.csv(df_all, "/home/doodlefish/Documents/Research/LepageLab/immunologyAndSz/Data/CCData/Daniel_2022-03-15_ProperEID.csv", row.names=FALSE)
# rm(df_all, eidToRemove) # remove dataframes not in use

````

```{r dataPrep, echo=FALSE}       
df_all <- read_csv("/home/doodlefish/Documents/Research/LepageLab/immunologyAndSz/Data/CCData/Daniel_2022-03-15_ProperEID.csv") # import df
         
cat(paste("There are ", sum(is.na(df_all$date_assess_t2)), "cases that have not completed time point 2. These cases will be removed."))
df_all <- df_all %>% 
  filter(!(is.na(df_all$date_assess_t2))) # remove rows without timepoint 2 assessment date
...
cogPCAVars <- c("cog_numMem_maxDigitRemem_t2", "cog_prospMem_timeDelay_t2", "cog_TMT_numericDuration_t2", "cog_TMT_numericErrors_t2", "cog_TMT_alphanumDuration_t2", "cog_TMT_alphanumErrors_t2", "cog_matrix_cor_t2", "cog_fluidIntel_score_t2", "cog_prospMem_result_t2", "cog_reactiontime_mean_t2", "cog_tower_cor_t2", "cog_digsub_cor_t2")
...
df <- df %>%
  ...
  mutate(sleep_duration_t0 = case_when( # field 1160 ('sleep_duration'), -1 = Do not know; -3 = Prefer not to answer;
      sleep_duration_t0 == "Prefer not to answer" ~ "NA",
      sleep_duration_t0 == "Do not know" ~ "NA",
      TRUE ~ sleep_duration_t0
  )) %>%
...
## Medication
df <- df %>% 
  mutate("anyMedOfInterest" = case_when(
    if_any(any_of(medVars), function(x) (x == "TRUE")) ~ 1,
    if_all(all_of(medVars), function(x) (x == "FALSE")) ~ 0
  ))
# table(df$anyMed)
```
...
df <- df %>% mutate_at(all_of(factorVars), factor)
...
essentialVars <- c(demoVars, completionTimeVars, cogPCAVars) # list of variable names that must be complete in order for case to be retained
...
# View(df[cogVars])
```
Critical variables: `r colnames(df[essentialVars])`
`r nrow(df_removed)` unique cases have missing values for critical variables and will thus be removed.
...
xyTab <- xyTable(df$diet_cookedVeg_t0, df$diet_fruit_t0)
plot <- plot(xyTab$x,xyTab$y, cex = xyTab$x*1.5, pch = 20, col = rgb(.1,0.1,0.1,0.7), xlab = "fruit intake (pieces/day)", ylab = "cooked veg intake (tbsp/day)" )
