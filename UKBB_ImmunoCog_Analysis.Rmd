...
lmAssumptionTest <- function(df, dfModel, y){ # 'df' is dataframe. 'dfModel' is the lm model of interest. 'y' is the outcome variable in a string.
## Assumption tests for STEMModel ----------------
  ...
  # if any VIF value is above 10, this indicates that multicollinearity regression coefs
...
## Mediation analyses
!! Will need to determine how to include covariate variables in this !!
For help with interpretation, see PSYC 536 lab 5 script.
```{r mediationFunction, echo=FALSE}
# see psyc 536 lecture 5 notes
mediationFunction <- function(df, X, Y, M, C){
  # total effect
  totalEffectModel <- lm(Y ~ X, data = df)
  summary(totalEffectModel)
  
  # a path -- predict M from X
  aPathModel <- lm(M ~ X, data = df)
  summary(aPathModel)
  
  # b path & direct path -- predict Y from M and X
  bAndDirectPathModel <- lm(Y ~ X + M, data = df)
  summary(bAndDirectPathModel)
  
  a <- coef(PROMIS_reg_M)[2]
  a.se <- coefficients(summary(aPathModel))[2,2]
  
  b <- coef(bAndDirectPathModel)[3]
  b.se <- coefficients(summary(bAndDirectPathModel))[3,2]
  
  medci(a,b,a.se,b.se, rho=0, alpha=.05, plot=TRUE)
