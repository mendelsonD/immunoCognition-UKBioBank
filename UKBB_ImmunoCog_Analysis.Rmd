...
vars <- c(cogVars, crpVars)
df$`cog_numMem-maxDigitRemem_t2` <- as.numeric(df$`cog_numMem-maxDigitRemem_t2`)
...
PCA <- function(df, vars){
  ...
  ## PCA 1 - standardize data 
  df_z <- df[vars] %>% 
    as_tibble() %>% 
    mutate(across(where(is.numeric), scale))
  describe(df_z) # Find a way to ensure that the mean really is 0 and std == 1 
  
  ### PCA 2 - number of components to retain
  #### eigenvalue method (eigen value > 1, suggested to keep)
  z_eigenValues <- eigen(cor(df_z))$values
...
df <- df %>% # number of hours difference between cognitive assessment and MRI
  mutate(timeDif_brainHourMinusCogHourCompleted = df[124] - df[123])

hourColNums <- c(contains("_hourCompleted", vars = colnames(df)),contains("_hourCollected", vars = colnames(df)))

df_hours <- df[hourColNums]
hourDescription <- py$descriptiveStats(df[hourColNums], toSave = F)
brainHourMinusCogHourCompleted)
describe(df$timeDif_brainHourMinusCogHourCompleted)
hist(df$timeDif_brainHourMinusCogHourCompleted)

table(df$crp_hourCollected)
describe(df$crp_hourCollected)
hist(df$crp_hourCollected)
mean(df$crp_hourCollected, na.rm = T)
sd(df$crp_hourCollected, na.rm = T)
...
essentialVars <- c(demoVars, completionTimeVars) # list of variable names that must be complete in order for case to be retained
