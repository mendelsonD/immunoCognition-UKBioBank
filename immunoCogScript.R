...
## Data Preparation -----
df <- read_csv("PATH/TO/DATA.csv") # import df
eidToRemove <- read_csv("PATH/TO/ListOfEIDsToRemove.csv", col_names = "eid") # file with EID of participants who withdrew consent from UKBB

df <- df %>% filter(!(eid %in% eidToRemove)) # remove rows with eid in this file
write.csv(df, "./data/{dataDate}_ProperEID.csv", row.names=FALSE)
...
## Subset ----
subsetData_outputPath <- "./outputs/Processed/Subsets"
...
## Summarise
write.csv(df_all, file = paste("./outputs/dfRetained_", dataDate, ".csv", sep = ""))
df <- df_all
...
write.csv(summary(df[-numVars]), file = paste("./outputs/descriptive/descriptive/df_all_summary_fctrVars", dataDate, ".csv", sep = "")) # save counts of categorical variables
