...
createBrainFigure <- function(file, figureValue, savePath, Y){
  ...
  if(figureValue == "p"){
    ...
    figureName <- glue("{Yname_file}_p")
    title <- "p-value"
    
    lowBound <- 0
    hiBound <- 1
    gradientCodes <- c("#062635", "#0A4C6A", "#12719E", "#1696D2", "#73BFE2","#A2D4EC","#CFE8F3") # Lower is better; full colour scale: c("#062635", "#0A4C6A", "#12719E", "#1696D2", "#46ABDB","#73BFE2","#A2D4EC","#CFE8F3")
    breakValues <- c(.01, .05, .1, .2, .5)
    breakValueLabels <- c(".01", ".05", ".1", ".2", ".5")
    
  } else if(figureValue == "pCor") {
    
    valueName <- "corrected p of indirect effect"
    value <- glue("{Y}.IndEff_p_cor")
    figureName <- glue("{Yname_file}_pCor")
    ...
    figureName <- glue("{Yname_file}_indEff_est_cor")
    ...
    breakValueLabels <- c(".001", ".01", ".05", ".1", ".2", ".5", ".9")
  ...
  DKTFigure <- reallyShort_DK %>%
    ...
    theme(
      ...
      legend.text = element_text(angle = 270, vjust = 1, hjust = 0.1),
      legend.text.align = .2,
...
subset <- c("noDxNoSSRI") # for testing purposes
figureValueToPlot <- "p" # either 'p', 'indEff', or 'pCor'
...
for(subset in subset){
  ...
  for(i in 1:length(formattedFile)){
    ...
    createBrainFigure(file = as.data.frame(formattedFile[i]), figureValue = figureValueToPlot, savePath = "./OutputFiles/Figures", Y = cogVar)
  }
